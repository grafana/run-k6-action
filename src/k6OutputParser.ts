...
const REGEX_EXPRESSIONS = {
    ...
    runningIteration:
      /running \(.*\), \d+\/\d+ VUs, \d+ complete and \d+ interrupted iterations/g,
...
export function parseK6Output(
  data: Buffer,
  testRunUrlsMap: TestRunUrlsMap | null,
  ...
  if (debug) {
    ...
    const filteredLines = lines.filter((line) => {
      const isRegexMatch = TEST_RUN_PROGRESS_MSG_REGEXES.some((regex) =>
        regex.test(line)
      )
