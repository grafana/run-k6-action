...
describe('run function', () => {
  ...
  it('should run k6 tests successfully with parallel=true', async () => {
    ...
    vi.mocked(k6helper.validateTestPaths).mockResolvedValue(['test1.js', 'test2.js']);
    vi.mocked(k6helper.isCloudIntegrationEnabled).mockReturnValue(false);
    vi.mocked(k6helper.generateK6RunCommand).mockImplementation((path) => `k6 run ${path}`);
    ...
    vi.mocked(k6helper.executeRunK6Command).mockReturnValue(mockChildProcess);
    ...
    expect(k6helper.validateTestPaths).toHaveBeenCalledWith(['test1.js', 'test2.js'], []);
  ...
  it('should skip test execution if only-verify-scripts is true', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith('🔍 Only verifying scripts. Skipping test execution');
  });
  ...
  it('should handle failed tests with fail-fast=true', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith('🚨 Fail fast enabled. Stopping further tests.');
  ...
  it('should handle failed tests with fail-fast=false', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith('🚨 Test failed with code: 1 and signal: ');
  ...
  it('should handle cloud integration with PR comment', async () => {
    ...
    const mockChildProcess: MockChildProcess = {
      on: vi.fn((event, callback) => {
        if (event === 'exit') {
          ...
          const testResultsMap = { 'test1.js': 'https://k6cloud.grafana.net/runs/123' };
    ...
    expect(k6helper.isCloudIntegrationEnabled).toHaveBeenCalled();
    expect(k6helper.generateK6RunCommand).toHaveBeenCalledWith('test1.js', '', true, false);
  });
});